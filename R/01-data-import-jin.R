...
threemonthsback <- (now() - days(60)) |> lubridate::format_ISO8601()
threeyearsback <- (now() - days(365 * 3)) |> lubridate::format_ISO8601()

api_list <- list(
  wfps_call_logs              = paste0("https://data.winnipeg.ca/resource/yg42-q284.json?$where=call_time>'", threemonthsback, "'&$group=neighbourhood,incident_type&$select=neighbourhood,incident_type,count(*)"),
  ...
  request311                 = paste0("https://data.winnipeg.ca/resource/u7f6-5326.json?$where=open_date>'", threemonthsback, "'&$group=neighbourhood,type&$select=neighbourhood,type,count(*)")
)
